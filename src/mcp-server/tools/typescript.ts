...
export const typescriptTools = (() => {
  ...
  const getOrCreateProject = withContext((ctx) => {
    if (!ctx.config.typescript.enabled) {
      throw new Error("TypeScript is not enabled")
    }

    project ??= new Project({
      tsConfigFilePath: ctx.config.typescript.tsConfigFilePath,
    ...
    defineTool(({ server, ...ctx }) =>
      ctx.config.typescript.enabled
        ? server.tool(
            ...
            (input, _extra) => {
              try {
                const project = getOrCreateProject(ctx)
                const results: {
                  filePath: string
                  sourceCode: string
...
export default typescriptTools
